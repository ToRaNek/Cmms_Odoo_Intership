...
class Model3D(models.Model):
    ...
    def import_hierarchy_from_gltf(self, gltf_data, parent_id=False, parent_disk_path=None):
        ...
        chaque enfant est dans models/<id_parent>/childs/<id_enfant>/
        """
        ...
        for idx, node in enumerate(nodes):
            ...
            # Détermine le chemin sur disque du parent (racine ou sous-childs)
            if parent_disk_path:
                submodel_disk_root = os.path.join(parent_disk_path, 'childs', str(submodel.id))
            else:
                parent_model = Model3D.browse(parent_id)
                submodel_disk_root = os.path.join(MODELS_DIR, str(parent_model.id), 'childs', str(submodel.id))
            ...
            # URL relative pour le web - changer ici aussi 'child' en 'childs'
            submodel.model_url = f"/models3d/{parent_id}/childs/{submodel.id}/{node_name}.gltf"
...
class Model3D(models.Model):
    ...
    def import_hierarchy_from_gltf(self, gltf_data, parent_id=False, parent_disk_path=None):
        ...
        for idx, node in enumerate(nodes):
            ...
            # Mise à jour des informations du modèle
            submodel.model_filename = f"{node_name}.gltf"
            # URL relative pour le web - utiliser le chemin childs
            submodel.model_url = f"/models3d/{parent_id}/childs/{submodel.id}/{node_name}.gltf"
